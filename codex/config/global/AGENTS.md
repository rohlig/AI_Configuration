...
- Delegated agents may use any model and effort supported by the current runtime, including Luna, Sol, Terra, GPT-6 Astra, or another available model. The parent agent should choose an appropriate route when useful; explicit model/effort settings for a delegated task take precedence over the primary default.
...
- Escalate only meaningful difficult portions by actually delegating a bounded subtask with the supported subagent tool. Use GPT-6 Astra for extreme end-to-end tasks, unusually difficult reasoning, or complex computer-control workflows when the runtime supports it; use Sol for difficult architecture, security, concurrency, cross-system reasoning, or central design judgment that does not require Astra. Start Astra or Sol at the lowest effort that can safely handle the task and increase only when justified. Do not use Terra by default; use it only when explicitly requested or when a measured, task-specific comparison shows it is the best fit. Return routine implementation to Luna when economical.
...
- A worker receiving that marker must remain in the current task until completion. A request to “act as Sol High” changes the worker's model/effort or review role; it does not authorize creating another task.
...
- GPT-6 Astra is reserved for extreme end-to-end tasks or complex computer-control workflows; it is an escalation route, not the default. Use the lowest suitable Astra effort and return to Luna for routine work.
