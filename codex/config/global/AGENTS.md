...
- A delegated worker must recognize that it is a worker, not the primary assistant: execute only the assigned bounded subtask, do not restart the task, reinterpret the user's request as a new direct task, or spawn another worker by default. Nested delegation requires an explicit instruction from the parent and a separately bounded scope; otherwise report the need back to the parent.
