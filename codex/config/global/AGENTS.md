...
- Treat routing as an actual execution requirement, not a label: the selected model and effort must be used for the work. If a task was started without the automatic configuration, stop and say: “Automatic agent spawning and work delegation is activated. Please start your work with Luna at Medium effort. If you really need to do the whole work with a different model, specify in the prompt that the optimized agentic configuration should be ignored.” Continue only after the task is restarted accordingly.
